# Filter-Related Hooks - Complete Documentation

Generated from: `/home/coding/vista/src/public/app.js`
...
This document provides complete code snippets and line numbers for all filter-related hooks in the vista application. These hooks handle various filtering operations including platform selection, metadata filtering, UI state management, and smart ordering interactions.
...
**Purpose:** Updates the set of enabled platforms in the cropper tool and refreshes the category legend overlay to reflect current selections.
...
**Purpose:** Updates the SVG overlay showing platform crop rectangles and safe zone intersection on the cropper tool.
...
**Purpose:** Renders the metadata table with filtering capability for tag names and values. Includes real-time filter input and export functionality.
...
**Purpose:** Updates the badge preview image and generates embed codes based on current score and selected style.
...
**Purpose:** Handles background type changes in the OG generator, showing/hiding relevant controls and updating the canvas.
...
**Purpose:** Handles logo position changes in the OG generator, toggling upload control visibility and updating the canvas.
...
**Purpose:** Updates the OG generator canvas by drawing background, content, and logo layers.
...
**Purpose:** Handles heatmap sorting by score or URL in ascending/descending order and re-renders the table.

---

## Platform Preferences Filter Hooks
...
**Purpose:** Toggles a platform's favorite status and updates the favorites list. Uses guardWrapper to protect against smart ordering conflicts.
...
**Purpose:** Determines whether a filter operation should be deferred based on smart ordering activity state.
...
**Purpose:** Queues a filter operation to be executed after smart ordering completes, with debug logging support.
...
**Purpose:** Processes all queued filter operations after smart ordering completes, with error handling and debug logging.
...
**Purpose:** Toggles a platform's hidden status and updates the UI. Uses guardWrapperWithRender to handle smart ordering interactions.
...
8109:     } catch (err) {
81010:       showToast('Failed to import preferences', 2000);
81011:     }
81012:   };
81013:   reader.readAsText(file);
81014:   e.target.value = ''; // Reset input
81015: }
```

**Purpose:** Imports platform preferences from a JSON file and applies them. Handles smart ordering deferral and UI updates.
...
**Purpose:** Toggles "What If" mode for testing metadata removal scenarios, with smart ordering awareness.
...
**Purpose:** Applies "What If" changes by removing selected tags from metadata and re-rendering previews, with accessibility announcements.
...
**Purpose:** Filters command palette commands by label or category based on user input.

---

## Summary

**Total filter-related hooks documented: 18**

- **Cropper Functions:** 2 (updateEnabledPlatforms, updateCropperOverlay)
- **Metadata Filtering:** 1 (renderMetadataTable)
- **Badge Preview:** 1 (updateBadgePreview)
- **OG Generator Functions:** 3 (handleBgTypeChange, handleLogoPosChange, updateOggenCanvas)
- **Heatmap Sorting:** 1 (handleHeatmapSort)
- **Platform Preferences:** 5 (toggleFavorite, toggleHidden, importPreferences, toggleWhatIfMode, applyWhatIfChanges)
- **Guard System Functions:** 4 (shouldDeferFilterOperation, isSmartOrdering, queueFilterOperation, processPendingFilterOperations)
- **Command Palette:** 1 (filterCommands)

Each hook includes complete code snippets with line numbers for reference and maintenance.
