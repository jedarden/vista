#!/usr/bin/env node
/**
 * Automated Platform Frame Screenshot Capture (Puppeteer)
 *
 * This script captures screenshots of all 7 platform context frame test pages
 * in both dark and light modes using Puppeteer (headless Chrome).
 *
 * Usage:
 ...
 *
 * Requirements:
 *   - Puppeteer installed (already in package.json dependencies)
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotsDir, `${platform.name}-dark.png`),
        fullPage: true
      });
      console.log(`   ✓ Dark mode: ${platform.name}-dark.png`);
      ...
      await page.screenshot({
        path: path.join(screenshotsDir, `${platform.name}-light.png`),
        fullPage: true
      });
      console.log(`   ✓ Light mode: ${platform.name}-light.png`);
