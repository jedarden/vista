...
/**
 * Light Theme Platform Screenshot Capture using ADB (Bead bf-49z20)
 *
 * This script captures screenshots of all 7 platform frames in light theme
...
const SERVER_URL = 'http://100.88.10.113:8082'; // Access server via Tailscale from phone
const OUTPUT_DIR = __dirname;
const THEME = 'light';

console.log('☀️ Light Theme Platform Screenshot Capture using ADB');
  ...
  for (const platform of PLATFORMS) {
    ...
    try {
      ...
      if (fs.existsSync(screenshotPath)) {
        ...
        if (stats.size > 80000) { // At least 80KB (light theme should be similar to dark theme)
          ...
          console.log(`   ❌ Screenshot too small (${stats.size} bytes) - may be blank or corrupted`);
          results.push({
            platform: platform.id,
            success: false,
...
(async () => {
  try {
    ...
    if (successful.length === PLATFORMS.length) {
      ...
      console.log('\nNext steps:');
      console.log('1. Verify file sizes are ~100KB (not 2.8KB)');
      console.log('2. Perform visual inspection of screenshots');
      console.log('3. Update verification report');
      console.log('4. Commit: git add screenshots/light-theme/*-light.png');
      console.log('5. Close bead: br close bf-49z20');
    }
