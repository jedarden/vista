...
/**
 * test-product-website-page-types-playwright.js
 *
 * Comprehensive test cases for product and website page type configurations using Playwright
 *
...
const { extractDomOrder, extractDomOrderDetailed } = require('./src/utils/extract-dom-order');
...
async function configurePageType(page, pageType) {
  ...
  await page.reload({ waitUntil: ['networkidle', 'load', 'domcontentloaded'] });
}
...
async function runPageTypeTest(config) {
  ...
  try {
    ...
    await page.goto(BASE_URL, { waitUntil: ['networkidle', 'load', 'domcontentloaded'] });
    ...
    // Extract actual platform order from DOM using the utility
    ...
    // Get detailed order for debugging
    const detailedOrder = await extractDomOrderDetailed(page);
    console.log('📋 Detailed platform info:');
    detailedOrder.slice(0, 10).forEach(item => {
    ...
    if (comparison.extra.length > 0) {
      ...
      comparison.extra.slice(0, 5).forEach(extra => {
        console.log(`   ${extra.platform} at position ${extra.platform}`);
      });
    }
    ...
    testResults.results = {
      ...
      urlSubmitted,
      detailedOrder: detailedOrder.slice(0, 10)
    };
