/**
 * Facebook Platform Preference Test - BF-5hjs5
 *
 * This test verifies that Facebook as a platform preference can be:
 * 1. Set via the settings UI
 * 2. Saved to localStorage
 * 3. Persisted across page reloads
 * 4. Reflected correctly in DOM ordering (Facebook appears first)
 *
 * Uses the same DOM inspection helpers as other platform preference tests.
 *
 * Usage: node test/integration/facebook-platform-preference-test.js
 */
...
<head>
  ...
  <title>Facebook Platform Preference Test</title>
  <style>
    body {
...
<body>
  <h1>Facebook Platform Preference Test</h1>
...
async function runTest() {
  console.log('\n' + '='.repeat(60));
  console.log('Facebook Platform Preference Test - BF-5hjs5');
  ...
  try {
    ...
    // Test 2: Setup - Change preferences (simulate setting Facebook as favorite)
    ...
    try {
      ...
      // Verify Facebook is first
      const facebookFirst = actualOrder[0] === TEST_PLATFORM;
      if (!facebookFirst) {
    ...
    console.log('\n📝 Test 4: Verify Platform Position Consistency');
    try {
      const orderWithFacebookFirst = [TEST_PLATFORM, ...ALL_PLATFORMS.filter(p => p !== TEST_PLATFORM)];
      const html = createTestHTML(orderWithFacebookFirst);
      const page = new MockPage(html);

      const actualOrder = await getPlatformOrder(page);
      const facebookIndex = actualOrder.indexOf(TEST_PLATFORM);

      console.log(`    Expected ${TEST_PLATFORM} at position: 1`);
      console.log(`    Actual ${TEST_PLATFORM} position: ${facebookIndex + 1}`);

      const positionMatches = facebookIndex === 0;

      if (!positionMatches) {
        throw new Error(`${TEST_PLATFORM} at wrong position: expected 1, got ${facebookIndex + 1}`);
      }
    ...
    if (failed === 0) {
      ...
      console.log('✅ Facebook platform preference can be set and verified');
    ...
    const resultsPath = path.join(resultsDir, `facebook-platform-preference-test.json`);
