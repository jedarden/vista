/**
 * Twitter Platform Preference Test - BF-5hjs5
 *
 * This test verifies that Twitter as a platform preference can be:
 * 1. Set via the settings UI
 * 2. Saved to localStorage
 * 3. Persisted across page reloads
 * 4. Reflected correctly in DOM ordering (Twitter appears first)
 *
 * Uses the same DOM inspection helpers as other platform preference tests.
 *
 * Usage: node test/integration/twitter-platform-preference-test.js
 */
...
<head>
  ...
  <title>Twitter Platform Preference Test</title>
  <style>
    body {
...
<body>
  <h1>Twitter Platform Preference Test</h1>
...
async function runTest() {
  console.log('\n' + '='.repeat(60));
  console.log('Twitter Platform Preference Test - BF-5hjs5');
  ...
  try {
    ...
    // Test 2: Setup - Change preferences (simulate setting Twitter as favorite)
    ...
    try {
      ...
      // Verify Twitter is first
      const twitterFirst = actualOrder[0] === TEST_PLATFORM;
      if (!twitterFirst) {
    ...
    console.log('\n📝 Test 4: Verify Platform Position Consistency');
    try {
      const orderWithTwitterFirst = [TEST_PLATFORM, ...ALL_PLATFORMS.filter(p => p !== TEST_PLATFORM)];
      const html = createTestHTML(orderWithTwitterFirst);
      const page = new MockPage(html);

      const actualOrder = await getPlatformOrder(page);
      const twitterIndex = actualOrder.indexOf(TEST_PLATFORM);

      console.log(`    Expected ${TEST_PLATFORM} at position: 1`);
      console.log(`    Actual ${TEST_PLATFORM} position: ${twitterIndex + 1}`);

      const positionMatches = twitterIndex === 0;

      if (!positionMatches) {
        throw new Error(`${TEST_PLATFORM} at wrong position: expected 1, got ${twitterIndex + 1}`);
      }
    ...
    if (failed === 0) {
      ...
      console.log('✅ Twitter platform preference can be set and verified');
    ...
    const resultsPath = path.join(resultsDir, `twitter-platform-preference-test.json`);
